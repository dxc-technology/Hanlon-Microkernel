...
# built from the current contents of the Occam-Microkernel project using the
# build-dependency-files.sh shell script, which is part of that same project)
for file in mk-open-vm-tools.tar.gz mk-custom-busybox.tar.gz mk-ipmi-mods.tar.gz \
    mk-ipmi-extensions.tar.gz occam-microkernel-overlay.tar.gz ssh-setup-files.tar.gz; do
